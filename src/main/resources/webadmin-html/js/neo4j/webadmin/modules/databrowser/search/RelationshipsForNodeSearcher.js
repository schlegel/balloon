  ...
  define(["neo4j/webadmin/utils/ItemUrlResolver"], function(ItemUrlResolver) {
    ...
    return RelationshipSearcher = (function() {
      function RelationshipSearcher(server) {
        ...
        this.pattern = /^((rels)|(relationships)):([0-9]+)$/i;
      }
      ...
      RelationshipSearcher.prototype.exec = function(statement) {
        return this.server.node(this.urlResolver.getNodeUrl(this.extractNodeId(statement))).then(function(node, fulfill) {
          return node.getRelationships().then(function(relationships) {
            return fulfill(relationships);
          });
        });
      };
      RelationshipSearcher.prototype.extractNodeId = function(statement) {
        var match;
